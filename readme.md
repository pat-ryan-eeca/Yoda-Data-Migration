...
Steps to perfrom to migrate LEHVF  from GEM to Enquire
1. Log on to the GEM databases server and run the master database script : one of

ExportAllLEHVF.sql

2. This will generetae oputput folders on the server, copy these to your working directory

3. Get the latest List for LEHVF and copy them to a subdirectory called ./List in the working folders
4. From a command shell in the working directory, run the powershell script
enrichLEHVSupplierList.ps1
5. run the script conactBankAccoutnt.ps1
